...
/* 绘制字符 */
static inline void draw_char(uint16_t *fb, uint16_t x, uint16_t y, 
                            char ch, uint16_t color, uint16_t bg_color,
                            pFONT *font) {
...
AkieGUI_Widget_T* AkieGUI_Button_Create(uint16_t x, uint16_t y, 
    ...
    /* 颜色配置 */
    priv->text_color = 0xFFFF;    /* 白色文字 */
    priv->bg_color = 0x001F;      /* 蓝色背景 */
    priv->press_color = 0x0010;    /* 深蓝按下 */
    priv->font = &ASCII_8x16;      /* 默认字体 */
